...
def get_first_order_polyfit(x, y, x_name = None, col = None):
    coeffs = np.polyfit(x, y, 2)
    a = round(coeffs[0], 6)
    b = round(coeffs[1], 6)
    v = [a, b]
    if x_name and col:
        print(f"({v[0]} * {x_name} + {v[1]}), // {col}")
    return v


def get_second_order_polyfit(x, y, x_name = None, col = None):
    coeffs = np.polyfit(x, y, 2)
    a = round(coeffs[0], 6)
    b = round(coeffs[1], 6)
    c = round(coeffs[2], 6)
    v = [a, b, c]
    if x_name and col:
        print(f"({v[0]} * std::pow({x_name}, 2) + {v[1]} * {x_name} + {v[2]}), // {col}")
    return v


def get_third_order_polyfit(x, y, x_name = None, col = None):
    ...
    v =  [a, b, c, d]
    if x_name and col:
        print(f"({v[0]} * std::pow({x_name}, 3) + {v[1]} * std::pow({x_name}, 2) + {v[2]} * {x_name} + {v[3]}), // {col}")
    return v

def get_val_from_fit(factor, terms):
    if len(terms) == 2:
        return terms[0] * factor + terms[1]
    elif len(terms) == 3:
        return terms[0] * pow(factor, 2) + terms[1] * factor + terms[2]
    else:
        return terms[0] * pow(factor, 3) + terms[1] * pow(factor, 2) + terms[2] * factor + terms[3]

def label_to_number(label):
    array = re.findall(r'[0-9]+', label)
    return float(array[0])

def find_interp_weights(factor, fit_set):
    factor_2_set = [label_to_number(k) for k in fit_set.keys()]
    weights = {}
